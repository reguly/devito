...
src.coordinates.data[1, :] = np.array(model.domain_size) * .05
src.coordinates.data[1, -1] = 20  # Depth is 20m
src.coordinates.data[2, :] = np.array(model.domain_size) * .05
...
f = TimeFunction(name="f", shape=model.grid.shape, grid=model.grid,
                 dimensions=model.grid.dimensions, space_order=so, time_order=2)
